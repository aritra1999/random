<script lang="ts">
    ...
    async function calculate() {
        ...
        conversionRate = await fetchConvertionRate();
...
<div>
    ...
    <div class="flex my-2 space-x-2">
        <input class="input w-52" type="number" data-type="currency" placeholder="€4.000" bind:value={salary} required/>
        ...
        <div>
            ...
            <div>
                <div class="flex items-center justify-between">
                    ...
                    <h1 class="item-heading">
                        🇮🇳 - {inrFormatter.format(details.salary * conversionRate)}
                    </h1>
                </div>
                <hr>
                    ...
                    <table class="w-full text-sm text-right text-gray-500">
                        ...
                        <tbody>
                            <tr class="border-b border-gray-200 ">
                                ...
                                <td>
                                    {inrFormatter.format((details.salary*conversionRate)/12)}
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td >
                                    - {inrFormatter.format((details.salary - details.afterTax)*conversionRate)}
                                </td>
                                <td >
                                    - {eurFormatter.format((details.salary - details.afterTax)/12)}
                                </td>
                                <td >
                                    - {inrFormatter.format(((details.salary - details.afterTax)*conversionRate)/12)}
                                </td>
                            </tr>
                            <tr class="font-medium border-b border-gray-200">
                                ...
                                <td class="bg-blue-50">
                                    {inrFormatter.format(details.afterTax*conversionRate)}
                                </td>
                                <td class="bg-blue-50">
                                    {eurFormatter.format(details.afterTax/12)}
                                </td>
                                <td class="bg-blue-50">
                                    {inrFormatter.format((details.afterTax*conversionRate)/12)}
                                </td>
                            </tr>
                            <tr class="border-b border-gray-200">
                                ...
                                <td>
                                    - {inrFormatter.format(details.expenses.total*conversionRate)}
                                </td>
                                <td>
                                    - {eurFormatter.format(details.expenses.total/12)}
                                </td>
                                <td>
                                    - {inrFormatter.format((details.expenses.total*conversionRate)/12)}
                                </td>
                            </tr>
                            <tr>
                                ...
                                <td>
                                    {inrFormatter.format((details.afterTax - details.expenses.total)*conversionRate)}
                                </td>
                                <td>
                                    {eurFormatter.format((details.afterTax - details.expenses.total)/12)}
                                </td>
                                <td>
                                    {inrFormatter.format(((details.afterTax - details.expenses.total)*conversionRate)/12)}
                                </td>
                            </tr>
                        </tbody>
                    </table>
        ...
        <table>
            ...
            <tbody class="text-right">
                <tr>
                    ...
                    <td> {inrFormatter.format(details.expenses.rent*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.rent/12)} </td>
                    <td> {inrFormatter.format((details.expenses.rent*conversionRate)/12)} </td>
                </tr>
                <tr class="bg-gray-50">
                    <th> Groceries </th> 
                    <td> {eurFormatter.format(details.expenses.groceries)} </td>
                    <td> {inrFormatter.format(details.expenses.groceries*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.groceries/12)} </td>
                    <td> {inrFormatter.format((details.expenses.groceries*conversionRate)/12)} </td>
                </tr>
                <tr>
                    <th> Electricity </th> 
                    <td> {eurFormatter.format(details.expenses.electricity)} </td>
                    <td> {inrFormatter.format(details.expenses.electricity*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.electricity/12)} </td>
                    <td> {inrFormatter.format((details.expenses.electricity*conversionRate)/12)} </td>
                </tr>
                <tr class="bg-gray-50">
                    <th> Internet </th> 
                    <td> {eurFormatter.format(details.expenses.internet)} </td>
                    <td> {inrFormatter.format(details.expenses.internet*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.internet/12)} </td>
                    <td> {inrFormatter.format((details.expenses.internet*conversionRate)/12)} </td>
                </tr>
                <tr>
                    <th> Mobile </th> 
                    <td> {eurFormatter.format(details.expenses.mobile)} </td>
                    <td> {inrFormatter.format(details.expenses.mobile*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.mobile/12)} </td>
                    <td> {inrFormatter.format((details.expenses.mobile*conversionRate)/12)} </td>
                </tr>
                <tr class="bg-gray-50">
                    <th> Misc </th> 
                    <td> {eurFormatter.format(details.expenses.miscellaneous)} </td>
                    <td> {inrFormatter.format(details.expenses.miscellaneous*conversionRate)} </td>
                    <td> {eurFormatter.format(details.expenses.miscellaneous/12)} </td>
                    <td> {inrFormatter.format((details.expenses.miscellaneous*conversionRate)/12)} </td>
                </tr>
            </tbody>
        </table>
    </div>
